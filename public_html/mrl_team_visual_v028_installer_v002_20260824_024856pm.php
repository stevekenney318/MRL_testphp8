 ...
 *
 * VERSION: v002
 * LAST MODIFIED: 8/24/2026 2:48:56 pm
 *
 * PURPOSE
 * -------
 * Correct the visual-only issues introduced with team.php v027:
 *
 * - Widen the form/pick panel without shrinking nested tables.
 ...
 * ------
 * - TESTPHP8-only
 * - Marker-based preflight (not fragile whole-block CSS matching)
...
$backupDir = $root . '/mrl_team_visual_v028_backup_20260824_024856pm';
...
function mrlv_replace_css_rule_once($content, $selector, $newRule)
{
    $pattern = '/(^|\n)([ \t]*)' . preg_quote($selector, '/') . '\s*\{[^}]*\}/s';
    $count = preg_match_all($pattern, $content, $m);
    if ($count !== 1) {
        throw new RuntimeException('CSS selector ' . $selector . ' expected exactly once; found ' . $count . '.');
    }
    ...
    if ($result === null || $replaced !== 1) {
        throw new RuntimeException('Unable to replace CSS selector ' . $selector . '.');
    }

    return $result;
}

$newPickPanelRule = <<<'CSS'
.mrl-pick-panel {
...
if ($preflightOk) {
    ...
    foreach (array(
        ...
        $pattern = '/(^|\n)[ \t]*' . preg_quote($selector, '/') . '\s*\{[^}]*\}/s';
        $count = preg_match_all($pattern, $current, $m);
        $preflightOk = mrlv_check(
            $checks,
            'CSS selector ' . $selector,
            $count === 1,
            'Expected selector exactly once; found ' . $count
        ) && $preflightOk;
    }
    ...
    $preflightOk = mrlv_check(
        $checks,
        'No v028 prior-year text override yet',
        mrlv_count($current, '.mrl-previous-years-content table,') === 0,
        'Expected new v028 rule to be absent before install'
    ) && $preflightOk;
}

function mrlv_build_v028(
    $content,
    $newPickPanelRule,
    $newPreviousRule,
    $newSummaryRule,
    ...
    $content = mrlv_replace_css_rule_once($content, '.mrl-pick-panel', $newPickPanelRule);
    $content = mrlv_replace_css_rule_once($content, '.mrl-previous-years', $newPreviousRule);
    $content = mrlv_replace_css_rule_once($content, '.mrl-previous-years summary', $newSummaryRule);
    $content = mrlv_replace_css_rule_once($content, '.mrl-previous-years-content', $newContentRule);

    $styleClose = "\n    </style>";
    if (substr_count($content, $styleClose) !== 1) {
        throw new RuntimeException('Expected exactly one closing style tag insertion point.');
    }
    $content = str_replace($styleClose, $tableTextRule . $styleClose, $content);

    $content = mrlv_replace_once(
        $content,
        $oldPrevBlock,
        $newPrevBlock,
        'previous years wrapper relocation'
    );
...
if ($action === 'install' && empty($errors)) {
    if (!$preflightOk) {
        ...
        try {
            ...
            $postOk = mrlv_check($checks, 'POST form width correction', mrlv_count($installed, 'width: 125%;') === 1, 'wider form panel installed') && $postOk;
            $postOk = mrlv_check($checks, 'POST prior-year dark text rule', mrlv_count($installed, '.mrl-previous-years-content table,') === 1, 'dark table text rule installed') && $postOk;
            ...
            if ($postOk) {
                $messages[] = 'PASS: team.php v028 installed. Visual-only correction complete; LP→RP functionality unchanged.';
            } else {
...
<div class="banner">
<h1>MRL team.php Visual Correction Installer v002</h1>
...
v002 change: preflight now validates the individual CSS selectors instead of requiring a whole CSS block to match byte-for-byte.<br>
After PASS, test both states again: pick window OPEN and CLOSED, then expand Previous Years in each.
</div>
