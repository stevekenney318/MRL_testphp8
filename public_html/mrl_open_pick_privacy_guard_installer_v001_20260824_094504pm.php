...
$backupDir = $root . '/mrl_open_pick_privacy_guard_backup_20260824_094504pm';
...
if ($preflightOk) {
    ...
    $preflightOk = pg_check($checks,'HTML + Excel use shared privacy flag',pg_c($chartCurrent,'if ($showSubmittedInsteadOfChart) {')>=2,'Both paths already honor flag') && $preflightOk;
...
if ($action==='install' && !$errors) {
    if (!$preflightOk) {
        ...
        try {
            ...
            $post=pg_check($checks,'POST Excel still gated',pg_c($ti,'if ($showSubmittedInsteadOfChart) {')>=2,'HTML and Excel protected') && $post;
...
<head>
<meta charset="utf-8">
<title>MRL Open Pick Privacy Guard v001</title>
<style>
...
<body><div class="wrap">
<div class="head"><h1>MRL Open Pick Privacy Guard Installer v001</h1>
