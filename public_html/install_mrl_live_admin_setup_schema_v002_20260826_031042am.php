 ...
 *
 * VERSION: v002
 * LAST MODIFIED: 8/26/2026 3:10:42 am
 *
 * CHANGELOG:
 * v002 (8/26/2026 3:10:42 am)
 * - Corrected TEST schema default-value normalization after preview showed false
 *   mismatches for MariaDB INFORMATION_SCHEMA defaults such as "'no'" and "NULL".
 * - Preflight now compares normalized defaults instead of raw driver strings.
 * - No migration scope changed: same nine admin_setup columns, same backup-first
 *   behavior, and segment_race_ranges still remains untouched.
 *
...
function asi_normalize_default($value) {
    if ($value === null) {
        return null;
    }
...
foreach ($expectedCols as $name => $exp) {
    ...
    $testOk = $testCol !== null
        ...
        && asi_normalize_default($testCol['COLUMN_DEFAULT']) === asi_normalize_default($exp['COLUMN_DEFAULT']);
        ...
        try {
            ...
            foreach ($expectedCols as $name => $exp) {
                ...
                $definitionOk = strtolower((string)$row['COLUMN_TYPE']) === strtolower($exp['COLUMN_TYPE'])
                    ...
                    && asi_normalize_default($row['COLUMN_DEFAULT']) === asi_normalize_default($exp['COLUMN_DEFAULT']);
