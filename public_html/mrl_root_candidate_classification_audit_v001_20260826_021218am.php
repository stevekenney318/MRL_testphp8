 ...
 *
 * VERSION: v001
 * LAST MODIFIED: 8/26/2026 2:12:18 am
 *
 * CHANGELOG:
...
function rcc_resolve_dependency(string $dep, string $baseRoot): array {
    ...
    if (strpos($clean, '/race_results/') === 0) {
        $candidate = dirname($baseRoot) . $clean;
        return [
            'dependency' => $dep,
...
foreach ($candidateNames as $name) {
    ...
    if ($classification === 'MIGRATE') {
        $dependencySummary[$name] = [
            ...
            'missing_count' => count(array_filter($resolvedDeps, static fn($d) => !$d['exists'])),
            'dependencies' => $resolvedDeps,
        ];
    }
}
...
$racesRefs = array_values(array_filter($referenceHits, static fn($h) => strtolower($h['needle']) === 'races.html'));
$countdownRefs = array_values(array_filter($referenceHits, static fn($h) => strtolower($h['needle']) === 'race_countdown.html'));
