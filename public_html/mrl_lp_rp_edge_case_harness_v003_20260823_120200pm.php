 ...
 * VERSION: v003
 * LAST MODIFIED: 8/23/2026 12:02:00 pm
 *
 * PURPOSE
 * -------
 ...
 *     team.php v024
 *     submit-team-picks.php v007 (verified only; not modified in Phase 1)
 ...
 * - PHASE 1 DOES NOT MODIFY submit-team-picks.php.
 * - Backs up team.php before changes.
...
if ($action === 'remove_files' && empty($errors) && $installedState) {
    try {
        if (!is_file($teamBackup)) {
            throw new RuntimeException('Original team.php backup is missing. Refusing cleanup.');
        }
        ...
        if (strpos($submitAfter, 'VERSION: v007') === false) {
            throw new RuntimeException('submit-team-picks.php unexpectedly changed from v007.');
        }
...
$installedNow =
    ...
    && strpos($submitNow, 'VERSION: v007') !== false
...
<div class="header">
<h1>MRL LP → RP EDGE-CASE TEST HARNESS v003</h1>
<div class="sub">TESTPHP8 ONLY • Phase 1: genuine LP baseline + shared-helper RP detection + public-form display • submit handler unchanged</div>
</div>
...
<tr><td>submit-team-picks.php</td><td>v007 unchanged in Phase 1</td></tr>
<tr><td>wrapper/helper</td><td>v010 / v005 unchanged</td></tr>
</table>

<div class="callout">
NEXT: Ctrl+F5 normal TESTPHP8 /team.php.<br>
Do NOT submit anything yet. Phase 1 intentionally leaves submit-team-picks.php untouched.<br>
...
Final reset after the full edge-case test: remove Phase-1 app test mode → restore TESTPHP8 DB from clean_baseline → verify normal pages → scheduler ON → WinSCP server→local → commit/push GitHub when appropriate.
</div>
