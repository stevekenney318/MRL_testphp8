 ...
 * VERSION: v002
 * LAST MODIFIED: 8/23/2026 11:55:00 am
 *
 * PURPOSE
 * -------
...
if (empty($errors) && $baselineState) {
    try {
        ...
        if (!preg_match('/function\s+mrl_rd_reject\s*\(\s*\)\s*:\s*void\s*\{/m', $submitSrc)) {
            throw new RuntimeException('submit-team-picks.php RD reject function not found.');
        }
        if (!preg_match('/\/\/ Deadline protection belongs on the server too, not just on team\.php\.\s*if\s*\(\s*!mrl_lp_effective_race_is_open\s*\(\s*\$raceYearInt\s*,\s*\$effectiveRace\s*\)\s*\)\s*\{\s*mrl_rd_reject\s*\(\s*\)\s*;\s*\}/m', $submitSrc)) {
            throw new RuntimeException('submit-team-picks.php real RD deadline gate not found.');
        }
        ...
        $rejectPattern = '/function\s+mrl_rd_reject\s*\(\s*\)\s*:\s*void\s*\{/m';
        $preparedSubmit = preg_replace(
            $rejectPattern,
            $submitHelper . '$0',
            $preparedSubmit,
            1,
        ...
        if ($preparedSubmit === null || $rejectReplaceCount !== 1) {
            throw new RuntimeException(
                'submit fixture helper: expected exactly 1 mrl_rd_reject function, found '
                . (int)$rejectReplaceCount
            );
        }
        $gatePattern = '/\s*\/\/ Deadline protection belongs on the server too, not just on team\.php\.\s*if\s*\(\s*!mrl_lp_effective_race_is_open\s*\(\s*\$raceYearInt\s*,\s*\$effectiveRace\s*\)\s*\)\s*\{\s*mrl_rd_reject\s*\(\s*\)\s*;\s*\}/m';
        $preparedSubmit = preg_replace(
            $gatePattern,
            "\n" . $submitTempGate,
            $preparedSubmit,
        ...
        if ($preparedSubmit === null || $gateReplaceCount !== 1) {
            throw new RuntimeException(
                'submit deadline gate: expected exactly 1 server deadline gate, found '
                . (int)$gateReplaceCount
            );
        }
    } catch (Throwable $e) {
...
<div class="header">
<h1>MRL LP → RP EDGE-CASE TEST HARNESS v002</h1>
