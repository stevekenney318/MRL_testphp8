 ...
 *
 * VERSION: v005
 * LAST MODIFIED: 8/24/2026 12:41:32 pm ET
 *
 * PURPOSE
 * -------
...
function ci_is_current_race_results_rp_item($name)
{
    $lower = strtolower((string)$name);

    return (
        $lower === '_rd_simulation'
        || $lower === 'admin_rd_simulation.php'
    );
}
...
if (empty($errors)) {
    ...
    if (is_dir($rr)) {
        ...
        if (is_array($rrEntries)) {
            foreach ($rrEntries as $name) {
                ...
                /*
                 * Current RP simulation assets remain available until the
                 * LP→RP reset/permanent finalization is complete.
                 */
                if (ci_is_current_race_results_rp_item($name)) {
                    ci_add_item(
                        $items,
                        $stats,
                        'KEEP UNTIL FINALIZATION',
                        ...
                        ci_modified_for_path($full),
                        ci_detail_for_path($full),
                        'Current Replacement Pick simulation asset; retain until reset/finalization.',
    ...
    if (is_dir($rdSimulation)) {
        /*
         * Count it for scope diagnostics only. The actual inventory row was
         * already added above as KEEP UNTIL FINALIZATION, avoiding duplicates.
         */
...
<div class="banner">
<h1>MRL TESTPHP8 NARROW CLEANUP INVENTORY v005</h1>
<div class="sub">
READ ONLY • generated 8/24/2026 12:41:32 pm ET • narrow development-artifact scan
</div>
...
<div class="ok">
<strong>v005 is intentionally narrow.</strong><br>
It does not recursively classify the whole TESTPHP8 site.
Normal WordPress, FormTools, DB backups, and normal race_results/YYYY/... contents are outside this cleanup pass.<br>
Current <span class="path">race_results/_rd_simulation</span> and <span class="path">race_results/admin_rd_simulation.php</span> are held in <strong>KEEP UNTIL FINALIZATION</strong>.
</div>
...
<li>Current RP simulation assets: <span class="path">race_results/_rd_simulation</span> and <span class="path">race_results/admin_rd_simulation.php</span> — retained until finalization</li>
