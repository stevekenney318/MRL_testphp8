 ...
 *
 * IMPORTANT CHANGE:
 * - Use GET (not POST) for "Show" to eliminate Chrome "Confirm Form Resubmission / ERR_CACHE_MISS"
 */
...
// ---------- action + REQUEST values (GET) ----------
$action = isset($_GET['action']) ? (string)$_GET['action'] : 'show';
if ($action !== 'show' && $action !== 'excel') {
    $action = 'show';
}

$reqYear    = isset($_GET['year']) ? (string)$_GET['year'] : '';
$reqSegment = isset($_GET['segment']) ? (string)$_GET['segment'] : '';

$hasRequest = (isset($_GET['year']) || isset($_GET['segment']) || isset($_GET['action']));
...
// ---------- selected values ----------
$selectedYear = (valid_year($reqYear) && in_array((string)$reqYear, $yearsStr, true))
    ? (string)$reqYear
    : $defaultYear;

$selectedSegment = (valid_segment($reqSegment) && in_array((string)$reqSegment, $segmentsStr, true))
    ? (string)$reqSegment
    : $defaultSegment;
...
$needsChartData = ($hasRequest && ($action === 'show' || $action === 'excel') && !$showSubmittedInsteadOfChart);
...
$isExcel = ($hasRequest && $action === 'excel');
...
<div class="teamchart-container">
    ...
    $chartDisplayed = ($hasRequest && $action === 'show' && !$showSubmittedInsteadOfChart && $dbError === '');
    ?>

    <!-- IMPORTANT: method="get" eliminates Chrome "Confirm Form Resubmission" -->
    <form id="teamchartForm" method="get" class="teamchart-form teamchart-no-print">
...
(function () {
    ...
    // Default action should always be "show"
    ...
    // Spreadsheet:
    // Use GET and open in a new tab so you stay on the chart page (no navigation away).
    if (btnExcel) {
        btnExcel.addEventListener('click', function () {
            const y = yearSel.value || '';
            const s = segSel.value || '';

            // Build URL: team_chart.php?action=excel&year=YYYY&segment=S#
            const url = new URL(window.location.href);
            url.searchParams.set('action', 'excel');
    ...
    // Print: set the title for PDF filename
    if (btnPrint) {
    ...
    // When the user clicks Show, make sure action is "show"
