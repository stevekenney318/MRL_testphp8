...
/**
 * install_race_finish_confirmation_v003_20260726_071639am.php
 * VERSION: v003
 * LAST MODIFIED: 7/26/2026 7:16:39 am
 * CHANGELOG:
 * - Installs source-specific Racing-Reference and Jayski monitoring.
 */
date_default_timezone_set('America/New_York');
$base=__DIR__;
$backup=$base . '/_race_finish_confirmation_install_backup_20260726_071639am';
...
header('Content-Type: text/plain; charset=UTF-8');
echo "MRL Race Finish Confirmation Monitor installer v003\n";
echo "Base: {$base}\n\n";
if(!is_file($schedulePath)) exit("ERROR: Missing {$schedulePath}\n");
...
if(!is_dir($backup)&&!@mkdir($backup,0755,true)&&!is_dir($backup)) exit("ERROR: Could not create backup.\n");
@mkdir($backup.'/_scheduler',0755,true);
if(!@copy($schedulePath,$backup.'/_scheduler/schedule.json')) exit("ERROR: Could not back up schedule.json.\n");
echo "BACKUP: _scheduler/schedule.json\n";
foreach($files as $relative=>$content){
 $target=$base.'/'.$relative; $dir=dirname($target);
 if(!is_dir($dir)&&!@mkdir($dir,0755,true)&&!is_dir($dir)) exit("ERROR: Could not create {$dir}\n");
 if(is_file($target)){ $bt=$backup.'/'.$relative; @mkdir(dirname($bt),0755,true); if(!@copy($target,$bt)) exit("ERROR: Could not back up {$relative}\n"); echo "BACKUP: {$relative}\n"; }
 $tmp=$target.'.tmp_'.bin2hex(random_bytes(4));
 if(@file_put_contents($tmp,$content,LOCK_EX)===false||!@rename($tmp,$target)){@unlink($tmp);exit("ERROR: Could not write {$relative}\n");}
 echo "INSTALLED: {$relative}\n";
}
@mkdir($base.'/_race_finish_confirmation/history',0755,true);
@mkdir($base.'/_race_finish_confirmation/raw',0755,true);
$schedule['tasks']['race_finish_confirmation_monitor']=[
 'enabled'=>true,'type'=>'interval','script'=>'race_finish_confirmation_monitor.php','args'=>['{year}'],'interval_minutes'=>1,'lock_minutes'=>2,'timeout_seconds'=>90,'run_method'=>'url','description'=>'Observation-only late-race secondary-source monitor. Reads existing MRL lap/status JSON and does not influence MRL decisions.'
];
$j=json_encode($schedule,JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES);
$tmp=$schedulePath.'.tmp_'.bin2hex(random_bytes(4));
if(!is_string($j)||@file_put_contents($tmp,$j."\n",LOCK_EX)===false||!@rename($tmp,$schedulePath)){@unlink($tmp);exit("ERROR: Could not update schedule.json. Restore from backup.\n");}
echo "UPDATED: _scheduler/schedule.json\n";
echo "\nSUCCESS\nNo second Hostinger cron is required.\nDashboard: race_finish_confirmation_dashboard.php\nManual test: race_finish_confirmation_monitor.php?force=1\nBackup: {$backup}\nDelete this installer after verification.\n";
