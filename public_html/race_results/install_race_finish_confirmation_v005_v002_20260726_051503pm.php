...
/**
 * install_race_finish_confirmation_v005_v002_20260726_051503pm.php
 *
 * VERSION: v002
 * LAST MODIFIED: 7/26/2026 5:15:03 pm
 *
 * PURPOSE:
 * - Corrected installer for race_finish_confirmation_monitor.php v005.
 * - Saves current MRL/ESPN race and lap progress below 90%.
 * - Keeps NASCAR Flag dependent on NASCAR live-source data.
 * - Keeps Finish Watch waiting below 90%.
 * - Does not contact Racing-Reference or Jayski before activation.
 *
 * PHP: 7.3 compatible.
 */
...
$backupDir = __DIR__ . '/_race_finish_confirmation_install_backup_20260726_051503pm';
$backupFile = $backupDir . '/race_finish_confirmation_monitor.php';

echo "MRL Race Finish Confirmation installer v005 v002\n";
echo "Base: " . __DIR__ . "\n\n";

if (!is_file($target)) {
    exit("ERROR: race_finish_confirmation_monitor.php was not found. Nothing changed.\n");
}

$source = @file_get_contents($target);
if (!is_string($source) || $source === '') {
    exit("ERROR: race_finish_confirmation_monitor.php could not be read. Nothing changed.\n");
}

if (strpos($source, "const RFCM_VERSION = 'v005';") !== false) {
    exit("ALREADY INSTALLED: race_finish_confirmation_monitor.php is already v005.\n");
}
...
foreach ($required as $pattern) {
    if (strpos($source, $pattern) === false) {
        exit("ERROR: expected v004 pattern missing:\n" . $pattern . "\nNothing changed.\n");
    }
}

if (!is_dir($backupDir) && !@mkdir($backupDir, 0755, true) && !is_dir($backupDir)) {
    exit("ERROR: backup directory could not be created. Nothing changed.\n");
}

if (!@copy($target, $backupFile)) {
    exit("ERROR: backup failed. Nothing changed.\n");
}
...
if ($c1 !== 1 || $c2 !== 1 || $c3 !== 1 || $c4 !== 1 || $c5 !== 1 || $c6 !== 1) {
    ...
    exit(
        "ERROR: unexpected update counts. Original restored.\n"
