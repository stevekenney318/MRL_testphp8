 ...
 * v011 (6/25/2026 2:34:45 pm)
 *   - NEW: Normal revision-monitor runs now verify/generate weekly standings release-version metadata for existing known snapshots.
 *   - NEW: Logs release-history metadata status so weekly_standings.php can rely on Current/Superseded and v1/v2/v3 display info.
 * v010 (6/21/2026)
...
// ------------------------- RELEASE-HISTORY VERSION METADATA VERIFY -------------------------
$releaseHistoryRefresh = [
    'ok' => false,
    'message' => 'Release-history helper unavailable.',
    'release_count' => 0,
];

if (function_exists('wsrel_ensure_history_version_metadata')) {
    $releaseHistoryRefresh = wsrel_ensure_history_version_metadata(__DIR__, (string)$year);
    rr_log_line(
        $logFile,
        'RELEASE HISTORY VERSION METADATA '
        . (!empty($releaseHistoryRefresh['ok']) ? 'OK' : 'WARN')
        . ' releases=' . (string)($releaseHistoryRefresh['release_count'] ?? 0)
        . ' changed=' . (!empty($releaseHistoryRefresh['changed']) ? 'YES' : 'NO')
        . ' message=' . (string)($releaseHistoryRefresh['message'] ?? '')
    );
    rrrev_out(
        'Release-history version metadata: '
        . (!empty($releaseHistoryRefresh['ok']) ? 'OK' : 'warning')
        . ' / Releases: ' . (string)($releaseHistoryRefresh['release_count'] ?? 0)
    );
} else {
    rr_log_line($logFile, 'RELEASE HISTORY VERSION METADATA SKIPPED: wsrel_ensure_history_version_metadata unavailable');
    rrrev_out('Release-history version metadata: skipped - helper unavailable.');
}
// ------------------------- SUMMARY -------------------------
rrrev_out("---");
