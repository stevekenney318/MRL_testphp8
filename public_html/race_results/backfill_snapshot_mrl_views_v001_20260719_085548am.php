...
VERSION: v001
LAST MODIFIED: 7/19/2026 8:55:48 am
...
{
    $headerRows = $xp->query('.//tr[th]', $table);
    ...
    foreach ($headerRows as $row) {
        ...
        for ($i = 0; $i < $cells->length; $i++) {
            ...
            if ($label === 'DRIVER' || strpos($label, 'DRIVER') !== false) {
                return $i;
            }
        }
    }
...
function mrlbf_filter_lite(
    ...
    if ($titleNodes !== false && $titleNodes->length) {
        ...
        if ($titleNode) {
            ...
            $titleNode->appendChild($dom->createTextNode($baseTitle . ' — ' . $viewLabel));
        }
    }
...
<style>
body{font-family:Arial,Helvetica,sans-serif;margin:24px;background:#f3f3f3;color:#111}
h1{margin:0 0 8px}
.panel{background:#fff;border:1px solid #bbb;border-radius:8px;padding:16px;margin:14px 0}
.summary{display:flex;gap:18px;flex-wrap:wrap;font-weight:700}
a.button{display:inline-block;padding:10px 14px;border-radius:6px;border:1px solid #555;background:#1769c2;color:#fff;text-decoration:none;font-weight:700}
