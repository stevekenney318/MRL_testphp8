 ...
 *
 * VERSION: v003
 * LAST MODIFIED: 8/26/2026 1:09:03 am
 *
 * CHANGELOG:
 * v003 (8/26/2026 1:09:03 am)
 * - Corrected false-positive active-reference scan for known cleanup utility files
 *   such as clean_race_finish_confirmation_data_v001_20260726_073602am.php.
 * - Cleanup utilities are treated as historical/admin tooling, not active runtime dependencies.
 * - INSTALL remains blocked for any other unexpected active runtime reference.
 *
...
const MRL_CONSOLIDATION_VERSION = 'v003';
...
function mrlc_scan_unexpected_active_refs(string $root): array
{
    ...
    foreach ($entries as $entry) {
        ...
        if (preg_match('/install|backup|\.bak|probe|diagnostic|audit|preflight|cleanup|clean_|debug/i', $entry)) {
            continue;
        }
