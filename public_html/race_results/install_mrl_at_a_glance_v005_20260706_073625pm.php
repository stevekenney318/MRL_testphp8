 ...
 * CHANGELOG:
 * v005 (7/6/2026 7:36:25 pm)
 * - Changed Scheduler/Race Monitor/Revision Monitor links to race_results_dashboard.php dashboard tabs.
 * - Improved race monitor text when the monitor is intentionally standing by between race windows.
 * - Keeps Latest Snapshot release/version labeling from v004.
 *
 * v004 (6/28/2026 12:33:29 pm)
...
function mrlg_monitor_row($task, $decision, $standbyLabel = '') {
    $next = (string)($decision['next_due_at'] ?? '');
    $interval = $decision['interval_minutes'] ?? $task['last_interval_minutes'] ?? null;
    $phaseLabel = trim((string)($decision['phase_label'] ?? ''));
    $due = !empty($decision['due']);

    if (!$due && is_numeric($interval) && (int)$interval <= 0) {
        $label = $standbyLabel !== '' ? $standbyLabel : 'Standby';
        return $label . ($phaseLabel !== '' ? ' • ' . $phaseLabel : '');
    }
...
$dashboard = $public . '/race_results_dashboard.php';
...
if ($heartbeatFile !== '') {
    ...
    $items[] = mrlg_item('scheduler', 'Scheduler', $status, 'Heartbeat ' . mrlg_age($age), basename($heartbeatFile), $dashboard);
} else {
    $items[] = mrlg_item('scheduler', 'Scheduler', 'red', 'Heartbeat not found', 'No scheduler heartbeat file found.', $dashboard);
}
...
if (!empty($raceTask)) {
    ...
    $text = mrlg_monitor_row($raceTask, $raceDecision, 'Standby');
    if (strpos($text, 'overdue') !== false && $status === 'green') $status = 'yellow';
    $items[] = mrlg_item('race_monitor', 'Race Monitor', $status, $text, (string)($raceTask['last_message'] ?? ''), $dashboard . '?tab=monitor');
} else {
    $items[] = mrlg_item('race_monitor', 'Race Monitor', 'yellow', 'No state found', '', $dashboard . '?tab=monitor');
}
...
if (!empty($revTask)) {
    ...
    if (strpos($text, 'overdue') !== false && $status === 'green') $status = 'yellow';
    $items[] = mrlg_item('revision_monitor', 'Revision Monitor', $status, $text, (string)($revTask['last_message'] ?? ''), $dashboard . '?tab=revision');
} else {
    $items[] = mrlg_item('revision_monitor', 'Revision Monitor', 'yellow', 'No state found', '', $dashboard . '?tab=revision');
}
