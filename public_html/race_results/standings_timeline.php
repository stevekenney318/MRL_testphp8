 ...
 *
 * VERSION: v004
 * LAST MODIFIED: 6/23/2026 1:38:33 am
 *
 * CHANGELOG:
 *
...
function st_render_weekly_winners_table(array $rows): void
{
    ...
    if (empty($rows)) {
        ...
        foreach ($rows as $row) {
            echo '<tr>';
            echo '<td class="st-racecol"><strong>' . st_h((string)($row['race_code'] ?? '')) . '</strong><br><span class="subtle">' . st_h((string)($row['race_label'] ?? '')) . '</span></td>';
            echo '<td>' . st_h((string)($row['team_name'] ?? '')) . '</td>';
...
<style>
    ...
    * { box-sizing: border-box; }
    body { margin: 0; padding: 18px; background: var(--bg); color: var(--text); font-family: var(--sans); font-size: 15px; }
    .wrap { max-width: 1540px; margin: 0 auto; }
    .hero { border: 1px solid var(--line); border-radius: 18px; padding: 16px; background: linear-gradient(180deg, rgba(213,180,255,0.13), rgba(255,255,255,0.025)); box-shadow: var(--shadow); margin-bottom: 14px; }
    h1 { margin: 0 0 4px 0; color: var(--accent); font-size: 31px; line-height: 1.1; }
    h2 { margin: 0 0 10px 0; color: var(--accent); font-size: 21px; line-height: 1.15; }
    .sub, .subtle { color: var(--muted); line-height: 1.35; }
    .controls { display: flex; flex-wrap: wrap; gap: 10px; align-items: center; margin-top: 14px; }
    select, .button, .navbtn { border: 1px solid var(--line); border-radius: 999px; background: #2d263e; color: var(--text); padding: 8px 12px; font-size: 14px; min-height: 36px; }
    select.snapshot-select { min-width: min(620px, 100%); max-width: 100%; }
    .race-select { min-width: 190px; }
    .button, .navbtn { display: inline-flex; align-items: center; justify-content: center; font-weight: 800; cursor: pointer; text-decoration: none; min-width: 54px; }
    .button:hover, .navbtn:hover { filter: brightness(1.08); text-decoration: none; }
    .navbtn.disabled { opacity: 0.35; cursor: default; pointer-events: none; }
    .snapshot-stepper { display: inline-flex; flex-direction: column; gap: 4px; align-items: stretch; justify-content: center; }
    .snapshot-stepper .snapbtn { min-width: 42px; min-height: 17px; height: 17px; padding: 0 10px; border-radius: 999px; font-size: 12px; line-height: 1; }
    .timeline-mode { display: inline-block; margin-left: 8px; color: #171421; background: var(--accent); border-radius: 999px; padding: 4px 10px; font-size: 13px; font-weight: 900; vertical-align: middle; }
    .version-tag { display: inline-flex; align-items: center; justify-content: center; min-width: 34px; border-radius: 999px; padding: 3px 8px; background: rgba(147,221,255,0.16); border: 1px solid rgba(147,221,255,0.45); color: var(--accent2); font-weight: 900; font-size: 12px; }
    .pill-row { display: flex; flex-wrap: wrap; gap: 8px; margin: 0 0 14px 0; }
    .pill { display: inline-flex; align-items: center; gap: 6px; border: 1px solid var(--line); border-radius: 999px; padding: 7px 11px; background: rgba(255,255,255,0.045); color: var(--text); line-height: 1.15; }
    .pill strong { color: var(--accent2); }
    .note { border: 1px solid rgba(252,211,77,0.42); border-radius: 14px; padding: 10px 12px; background: rgba(252,211,77,0.08); color: #fff8d6; margin-bottom: 14px; line-height: 1.35; }
    .grid { display: grid; grid-template-columns: 1fr; gap: 14px; }
    @media (min-width: 1150px) { .grid { grid-template-columns: repeat(4, minmax(0, 1fr)); } }
    .card, .wide-card { border: 1px solid var(--line); border-radius: 16px; padding: 14px; background: linear-gradient(180deg, var(--panel), var(--panel2)); box-shadow: var(--shadow); overflow: hidden; }
    .wide-card { margin-top: 14px; }
    .table-wrap { overflow-x: auto; }
    table { width: 100%; border-collapse: collapse; background: rgba(0,0,0,0.16); border-radius: 12px; overflow: hidden; }
    th, td { padding: 8px 9px; border-bottom: 1px solid rgba(255,255,255,0.08); text-align: left; line-height: 1.22; font-size: 14px; }
    th { color: var(--accent); background: rgba(0,0,0,0.22); font-size: 13px; }
    tr:last-child td { border-bottom: none; }
    .st-rank { width: 42px; text-align: right; color: var(--muted); }
    .st-score { width: 70px; text-align: right; font-weight: 800; }
    .st-racecol { width: 76px; }
    .st-empty { color: var(--muted); font-style: italic; text-align: center; }
    .snapshot-table td, .snapshot-table th { font-size: 12px; }
...
<div class="wrap">
    ...
    <div class="pill-row">
        <span class="pill"><strong>As of:</strong> <?php echo st_h($selectedSnapshotDisplay !== '' ? $selectedSnapshotDisplay : 'No snapshot selected'); ?></span>
        <span class="pill"><strong>As-of snapshot:</strong> <span class="mono"><?php echo st_h($selectedSnapshotValue !== '' ? $selectedSnapshotValue : 'none'); ?></span></span>
        <span class="pill"><strong>As-of version:</strong> <span class="version-tag"><?php echo st_h((string)($selectedSnapshot['version_label'] ?? '')); ?></span></span>
        <span class="pill"><strong>As-of race:</strong> <?php echo st_h(trim($asOfRaceCode . ' ' . $asOfRaceLabel)); ?></span>
        <span class="pill"><strong>Viewing race:</strong> <?php echo st_h(trim($selectedViewRaceCode . ' ' . $selectedViewRaceLabel)); ?></span>
    </div>

    <div class="note">
        <?php echo st_h($timelineSummary); ?> Demo limitation: pick rows currently come from the same official segment-pick source used by weekly standings today. Future LP/RD effective_race rules can be applied when that model is live.
    </div>
    ...
    <div class="footer">standings_timeline.php v004 · generated <?php echo st_h(date('n/j/Y g:i:s a')); ?></div>
</div>
<script>
