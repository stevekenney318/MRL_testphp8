 ...
 * VERSION: v002
 * LAST MODIFIED: 8/20/2026 2:15:00 am
 *
...
const PATCH_TS = '20260820_021500am';
...
if (!$errors) {
    try {
        ...
        $old = <<<'OLD'
            $segType = 'SEG';
            $sqlUpdate = "UPDATE user_picks SET pick_type=?, effective_race=? WHERE pickID=? AND pick_type='LP'";
            $stmtUpdate = mysqli_prepare($dbconnect, $sqlUpdate);
            if (!$stmtUpdate) {
                throw new RuntimeException('Unable to prepare current-pick update.');
            }

            mysqli_stmt_bind_param($stmtUpdate, 'sii', $segType, $segmentStart, $pickID);
OLD;
        ...
        $new = <<<'NEW'
            ...
            // Preserve the true original submission timestamp in the SAME
            // UPDATE that changes LP -> SEG so ON UPDATE behavior cannot
            // replace entryDate with the approval time.
            mysqli_stmt_bind_param($stmtUpdate, 'sisi', $segType, $segmentStart, $originalEntryDate, $pickID);
NEW;
...
<head>
<meta charset="utf-8">
<title>MRL Focused ADJ Patch v002</title>
<style>
...
<h1>MRL Focused ADJ Patch v002</h1>

<div class="card">
<b>Target:</b> <?php echo h($host); ?><br>
<b>Root:</b> <?php echo h($root); ?><br>
<b>Build:</b> 8/20/2026 2:15:00 am ET
</div>
...
<form method="post">
<button class="btn" name="action" value="install">INSTALL FOCUSED ADJ PATCH v002</button>
</form>
</div>
