...
/**
 * install_race_finish_confirmation_v004_20260726_092259am.php
 * VERSION: v004
...
$base=__DIR__;
$stamp='20260726_092259am';
$backup=$base.'/_race_finish_confirmation_install_backup_'.$stamp;
...
echo "MRL Race Finish Confirmation Monitor installer v004\nBase: $base\n\n";
if(!is_file($schedulePath)) exit("ERROR: schedule.json not found. Nothing changed.\n");
$scheduleRaw=file_get_contents($schedulePath); $schedule=json_decode((string)$scheduleRaw,true);
if(!is_array($schedule)||!isset($schedule['tasks'])||!is_array($schedule['tasks'])) exit("ERROR: invalid schedule.json. Nothing changed.\n");
if(!is_dir($backup) && !mkdir($backup,0755,true) && !is_dir($backup)) exit("ERROR: cannot create backup.\n");
mkdir($backup.'/_scheduler',0755,true); copy($schedulePath,$backup.'/_scheduler/schedule.json'); echo "BACKUP: _scheduler/schedule.json\n";
foreach($files as $rel=>$content){ $target=$base.'/'.$rel; $dir=dirname($target); if(!is_dir($dir)) mkdir($dir,0755,true); if(is_file($target)){ $bd=dirname($backup.'/'.$rel); if(!is_dir($bd)) mkdir($bd,0755,true); copy($target,$backup.'/'.$rel); echo "BACKUP: $rel\n"; } $tmp=$target.'.tmp_'.bin2hex(random_bytes(4)); if(file_put_contents($tmp,$content,LOCK_EX)===false||!rename($tmp,$target)) exit("ERROR writing $rel\n"); echo "INSTALLED: $rel\n"; }
// Clear prior runtime observations so v004 starts clean.
$data=$base.'/_race_finish_confirmation';
foreach(['latest.json','state.json','monitor.log','monitor.lock'] as $f){ if(is_file($data.'/'.$f)) unlink($data.'/'.$f); }
foreach(['history','raw'] as $d){ $dir=$data.'/'.$d; if(!is_dir($dir)) mkdir($dir,0755,true); foreach(scandir($dir)?:[] as $item){ if($item==='.'||$item==='..'||$item==='.gitkeep') continue; $p=$dir.'/'.$item; if(is_file($p)||is_link($p)) unlink($p); } }
echo "CLEARED: prior runtime observations/history/raw HTML\n";
$schedule['tasks']['race_finish_confirmation_monitor']=[
 'enabled'=>true,'type'=>'interval','script'=>'race_finish_confirmation_monitor.php','args'=>['{year}'],'interval_minutes'=>1,'lock_minutes'=>2,'timeout_seconds'=>90,'run_method'=>'url','description'=>'Observation-only MRL vs NASCAR vs secondary-source late-race monitor. Cup Series only; does not influence MRL decisions.'
];
$json=json_encode($schedule,JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES); $tmp=$schedulePath.'.tmp_'.bin2hex(random_bytes(4)); file_put_contents($tmp,$json."\n",LOCK_EX); rename($tmp,$schedulePath); echo "UPDATED: _scheduler/schedule.json\n\nSUCCESS\nNo second Hostinger cron is required.\nDashboard: race_finish_confirmation_dashboard.php\nBackup: $backup\nDelete this installer after verification.\n";
