...
const INSTALLER_TIMESTAMP = '20260819_071200pm';
...
if(!$errors){
    try{
        ...
        foreach(['user_picks','user_picks_history'] as $table){
            ...
            $typeOk=strpos($pickTypeDef,'LP')!==false && strpos($pickTypeDef,'SEG')!==false && strpos($pickTypeDef,'ADJ')!==false;
            $checks[]=[$table.'.pick_type supports LP / SEG / ADJ',$typeOk?'PASS':'FAIL'];
            if(!$typeOk) $errors[]=$table.'.pick_type missing LP/SEG/ADJ support.';
        }
...
?>
<!doctype html>
<html><head><meta charset="utf-8"><title>MRL LP Admin Adjustment Installer v001</title>
<style>
...
<h1>MRL LP Admin Adjustment Installer v001</h1>
<div class="card"><b>Target:</b> <?php echo ih($host); ?><br><b>Root:</b> <?php echo ih($root); ?><br><b>Build:</b> 8/19/2026 7:12:00 pm ET</div>
...
<form method="post"><button class="btn" name="action" value="install">INSTALL LP ADMIN ADJUSTMENT v001</button></form></div>
<?php endif; ?>
</div></body></html>
