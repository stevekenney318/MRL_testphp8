 ...
 * - Preflight does NOT depend on shell exec() or a server PHP CLI binary.
...
const INSTALLER_TIMESTAMP = '20260819_030800pm';
...
if (empty($errors)) {
    try {
        ...
        foreach ($prepared as $label => $content) {
            $ok = (strpos($content, '<?php') === 0)
                && (strpos($content, 'VERSION: v002') !== false)
                && (strlen($content) > 500);

            $preflight[] = [$label . ' generated replacement structure', $ok ? 'PASS' : 'FAIL'];
            if (!$ok) {
                $errors[] = $label . ' generated replacement structure check failed.';
            }
        }
