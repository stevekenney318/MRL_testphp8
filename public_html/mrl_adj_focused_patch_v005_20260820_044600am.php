 ...
 * VERSION: v005
 * LAST MODIFIED: 8/20/2026 4:46:00 am
 *
...
const PATCH_TS = '20260820_044600am';
...
NEW;
        ...
        // Validate the actual prepared semantics, not local variable names.
        $adminPrepared = $prepared['admin_pick_adjustment.php'];

        $hasAdjHistoryType =
            strpos($adminPrepared, "pick_type='ADJ'") !== false ||
            strpos($adminPrepared, "'ADJ'") !== false;

        $hasSegCurrentUpdate =
            strpos($adminPrepared, "SET pick_type=?, effective_race=?, entryDate=?") !== false &&
            strpos($adminPrepared, "WHERE pickID=? AND pick_type='LP'") !== false &&
            strpos($adminPrepared, "'SEG'") !== false;

        $hasEntryDatePreservation =
            strpos($adminPrepared, '$originalEntryDate = (string)$row[\'entryDate\'];') !== false &&
            strpos($adminPrepared, 'mysqli_stmt_bind_param($stmtU, \'sisi\'') !== false;

        $hasAdjHistoryInsert =
            strpos($adminPrepared, 'INSERT INTO user_picks_history') !== false &&
            strpos($adminPrepared, 'supersedes_pickID') !== false;
        ...
        if (!$semanticsOk) {
            if (!$hasAdjHistoryType) $errors[] = 'Semantics guard: ADJ history type marker not found.';
            if (!$hasSegCurrentUpdate) $errors[] = 'Semantics guard: LP -> SEG current-row update not found.';
            if (!$hasEntryDatePreservation) $errors[] = 'Semantics guard: entryDate preservation not found.';
            if (!$hasAdjHistoryInsert) $errors[] = 'Semantics guard: user_picks_history insert / supersedes link not found.';
        }
...
<head>
<meta charset="utf-8">
<title>MRL Focused ADJ Patch v005</title>
<style>
...
<h1>MRL Focused ADJ Patch v005</h1>

<div class="card">
<b>Target:</b> <?php echo h($host); ?><br>
<b>Root:</b> <?php echo h($root); ?><br>
<b>Build:</b> 8/20/2026 4:46:00 am ET
</div>
...
<form method="post">
<button class="btn" name="action" value="install">INSTALL FOCUSED ADJ PATCH v005</button>
</form>
</div>
